...
Provide identity resolution, policy evaluation, and workflow state machines for structured content systems. Designed to work with Verevoir but deliberately standalone — zero Verevoir dependencies — so it is useful in any TypeScript application that needs role-based access control or editorial workflows.

## Goals

- Standalone: usable without any other Verevoir package
...
- Store roles or permissions — roles come from the identity provider, not from Verevoir
...
- **Zero Verevoir dependencies.** Access control is a general concern, not specific to content management. Keeping the package standalone makes it useful beyond Verevoir and avoids coupling to the schema engine or storage adapter.
